...
const UsersTab = ({ users }) => {
  return (
    <section>
      <h3 className="text-neutral bg-base-100 py-4 mb-4 text-center text-2xl font-bold">
        Users
      </h3>
      <ul className="flex flex-col w-32">
        ...
        {users.map((user, index) => {
          return (
            <li
              key={index}
              className="bg-base-100 py-4 text-accent w-full text-center border-b"
            >
              {user.name}
            </li>
          );
...
const ChannelDashboard = () => {
  ...
  return (
    <>
      <Navbar />
      <div
        className="p-4 md:p-8 text-2xl font-bold text-secondary cursor-pointer uppercase"
