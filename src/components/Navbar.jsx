...
import CHRIST_LOGO from "../assets/CHRIST_LOGO.svg";
...
const NavHeader = () => {
  return (
    <div className="text-center px-2 md:px-8 flex justify-center">
      <Link
        to="/"
        className="my-auto text-xl md:text-2xl text-neutral font-bold"
      >
        <img src={CHRIST_LOGO} alt="Chat Logo" className="inline-block w-10 mr-3" />
        GUIDED
      </Link>
    </div>
  );
  ...
  return (
    <div className="flex flex-col md:flex-row justify-center items-center gap-4 md:ml-auto md:px-8">
      ...
      {state == "login" && !isLogin ? (
        <Link
          to="/auth/login"
          className="btn btn-sm md:btn-md btn-secondary btn-outline px-8"
        >
          Login
        </Link>
      ) : (
