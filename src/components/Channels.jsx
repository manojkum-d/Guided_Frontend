      ...
      <div className="modal modal-bottom sm:modal-middle overflow-hidden">
        <div className="modal-box overflow-hidden max-w-sm w-fit">
          ...
          <div className="form-control w-fit">
            ...
            <input
              type="text"
              placeholder="Type here the channel name"
              className="input input-bordered w-full max-w-xs"
              ref={nameRef}
            />
          </div>
          ...
          <div className="form-control w-fit">
            ...
            <input
              type="text"
              placeholder="Here goes you channel description"
              className="input input-bordered w-full max-w-xs"
              ref={descriptionRef}
            />
          </div>
...
const Channels = () => {
  ...
  const fetchIsInChannelList = useCallback(async (channelsId) => {
    ...
    channelsId.forEach(async (channelId) => {
      ...
      setIsInChannelList((isInChannelList) => {
        return { ...isInChannelList, ...isInChannel };
      });
    });
  }, []);
      ...
      {channels ? (
        <div className="flex flex-row flex-wrap gap-6 justify-between min-h-[90vh]">
          {channels.map((channel, index) => (
            <div
              className="card md:w-[calc(50%-1.5em)] lg:w-[calc(33.33%-1.5em)] bg-base-100 shadow-xl border"
              key={index}
            >
              <figure>
              ...
              <div className="card-body">
                ...
                {isInChannelList[channel.channelId] ? (
                  ...
                  <button
                    ...
                    onClick={(e) => handleJoinChannel(channel)}
                  >
                    Join
                  </button>
                )}
          ...
          {role === "admin" ? (
            <label
              htmlFor="my-modal-6"
              className="btn btn-secondary btn-xl mx-auto mt-8"
            >
              Create Channel
            </label>
          ) : (
