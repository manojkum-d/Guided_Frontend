...
const GroupsTab = ({ groups, channelId }) => {
  ...
  const { activeIndex, setActiveIndex, activeGroupId, setActiveGroupId } =
    useContext(GroupContext);

  return (
    <section>
      <h3 className="text-neutral mb-4 text-center text-2xl font-bold bg-base-100 py-4">
        Groups
      </h3>
      <ul className="flex flex-col w-48 h-[100%]">
        ...
        {groups.map((group, index) => {
          return (
            <li
              key={index}
              className={`px-8 py-4 text-accent text-center border-b cursor-pointer ${
                activeIndex == index
